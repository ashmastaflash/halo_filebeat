...
class HaloEvents(object):
    ...
    def __iter__(self):
        while True:
            HaloEvents.replace_last_event_tstamp(str(self.last_event_timestamp))
    ...
    @classmethod
    def replace_last_event_tstamp(cls, tstamp):
        tstamp_file = "/tmp/halo_events_tstamp"
        with open(tstamp_file, 'w') as ts_f:
            ts_f.write(tstamp)

    @classmethod
    def replace_last_error(cls, err):
        halo_err_file = "/tmp/halo_events_error"
        with open(halo_err_file, 'w') as h_err_f:
            h_err_f.write(err)
    ...
    def get_pages(self, url_list):
        ...
        try:
            ...
            err_msg = "%s | %s" % (str(self.last_event_timestamp), str(e))
            HaloEvents.replace_last_error(err_msg)
            del pool
            # We sleep for 10s to give the grid some time to cool off
            time.sleep(10)
        except cloudpassage.exceptions.CloudPassageGeneral as e:
            err_msg = "%s | %s" % (str(self.last_event_timestamp), str(e))
            HaloEvents.replace_last_error(err_msg)
            # Grid is HAWT.  Chill for a few.
            del pool
            time.sleep(10)
        return results
